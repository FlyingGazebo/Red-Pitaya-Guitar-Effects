/**
 * $Id: red_pitaya_guitar_distortion.v$
 *
 ...
 */
 ...
 module red_pitaya_guitar_distortion
 (
    input clk_i ,
    input rstn_i,
...
reg [ 9-1: 0] ip       ;
reg [16-1: 0] big_out  ;
wire [16-1: 0] in_signed = ~in_sound + 1'b1; //two's complement by hand
...
begin
    ...
    begin
        ...
        if (in_sound == 16'h8000) //overflow on 2^15
            abs_sound <= 16'hffff;
        else
        begin
            if (in_sound[15] == 1'b0)
        ...
        ip    <= {1'b0, abs_sound[ 7:0]}; //sets distance from the point in table to actual used interpolated point
    ...
    mult_unn <= table_d*ip;    
    abs_out <= table_y + {1'b0, mult_unn[15:4]}; //truncate lower bits 
    ...
    begin
        neg_out <= ~abs_out+1'b1;
        if (asym_i)
            big_out <= {2'b11, neg_out[14:1]}; //halve the below-zero part of wave
        else
            big_out <= neg_out;
    end
end

assign out_distortion = big_out;
